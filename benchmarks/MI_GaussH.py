...
filename = "redo_GaussH.pkl"
Nvars = [9,8,7,6,5,4,3,2]
mc_size = 1500
...
likelihoodU = [bvcopula.IndependenceCopula_Likelihood(),
                ...
                bvcopula.GaussianCopula_Likelihood()] # True unconditional
...
for Nvar in Nvars:
	...
	for repetition in range(Rps):
		...
		t1 = time.time()
		print(f"Estimated in {(t1-t0)//60} min (int {(t01-t0)//60} min, est {(t1-t01)//60} min)")
